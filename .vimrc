...
if &runtimepath =~ 'vim-go'
	def g:SetGoOptions()
		...
		g:go_gopls_options = ['-remote=auto', '-logfile=/tmp/gopls.log', '-remote.logfile=/tmp/gopls2.log']
		g:go_gopls_local = {
			"build.env": {"GOPACKAGESDRIVER": "/home/xa/.vim/tools/gopackagesdriver.sh"},
			'build.directoryFilers': [
				'-bazel-bin',
				'-bazel-out',
				'-bazel-testlogs',
				'-bazel',
	...
	augroup golang
		autocmd!
		au FileType go call g:SetGoOptions()
...
" Don't start at the top if search is not found anymore.
set nowrapscan
...
" test chatgpt
def! ChatGPT()
	var apiKeyPath = expand("~/.vim/.private/chatgpt.vim")
	if filereadable(apiKeyPath)
		exe "source" apiKeyPath
	endif
enddef
...
runtime START myyankstack.vim
if exists("g:loaded_myyankstack") && g:loaded_myyankstack == 1
	call g:MyyankstackSetup()
...
" Copilot
imap <C-L>w <Plug>(copilot-accept-word)
